...
const logFormat = format.printf(info => `${info.timestamp.split(' ')[1]} ${info.level}: ${JSON.stringify(info.message, null, 4)}`)
...
const logger = createLogger({
    ...
    transports: [
        ...
        new winston.transports.File({ filename: '/var/log/combined.log', format: format.combine(format.json()) })
    ]
})
